...
result <- Proposed_Aggreg(k = 50, SNP_names = obj.bigSNP$map$marker.ID[indx], Gusing = G_using, POS_using = POS_using, CHR_using = CHR_using, effective_gene = POS_EFF, bE = 0.3, bint = 0.3, bG = 0.8, sigmaE = 1, sigma_eps = 0.5)
...
#1:
error1 <- Compare_Aggreg_type1_error(SNP_names = obj.bigSNP$map$marker.ID[indx], G_using = G_using, POS_using = POS_using, CHR_using = CHR_using, effective_gene = POS_EFF1, bE = 0.3, bint = 0.5, bG = 0.8, sigmaE = 1, sigma_eps = 0.5)
error1 <- error1 %>% filter(BP != POS_EFF1) %>% select(P:oracle_error)
r1 <- apply(error1<= 5 * (10 ^ -2), 2, mean)


#3:
error2 <- Compare_Aggreg_type1_error(SNP_names = obj.bigSNP$map$marker.ID[indx], G_using = G_using, POS_using = POS_using, CHR_using = CHR_using, effective_gene = POS_EFF2, bE = 0.3, bint = 0.5, bG = 0.8, sigmaE = 1, sigma_eps = 0.5)
error2 <- error2 %>% filter(BP != POS_EFF2) %>% select(P:oracle_error)
r2 <- apply(error2<=5 * (10 ^ -2), 2, mean)

#5:
error3 <- Compare_Aggreg_type1_error(SNP_names = obj.bigSNP$map$marker.ID[indx], G_using = G_using, POS_using = POS_using, CHR_using = CHR_using, effective_gene = POS_EFF3, bE = 0.3, bint = 0.5, bG = 0.8, sigmaE = 1, sigma_eps = 0.5)
error3 <- error3 %>% filter(BP != POS_EFF3) %>% select(P:oracle_error)
r3 <- apply(error3<=5 * (10 ^ -2), 2, mean)


r <- as_tibble(rbind(r1,r2,r3))
r$MAF <- sort(MAF[POS_using %in% c(POS_EFF1,POS_EFF2,POS_EFF3)])
r %>% rename(c("proposed" = "P","oracle" = "oracle_p")) %>% pivot_longer(proposed:oracle_error, "Type", values_to = "Error") %>% ggplot(aes(MAF,Error,color = Type)) + geom_line() + ylab("Type I Error") + geom_hline(aes(yintercept = 0.05))
...
```{r}
all_error <- as_tibble(error3<=0.05)
all_error <- all_error %>% mutate(MAF = MAF[!POS_using %in% c(POS_EFF5)])
...
all_error <- all_error %>% group_by(MAF_level) %>% summarise(proposed = mean(P), oracle = mean(oracle_p), oracle_error = mean(oracle_error), number = n())
...
### Compute type I error rate of each case:
#1:
suppressWarnings(error1 <- Compare_Aggreg_type1_error_auxiliary(G_using = G_using, POS_using = POS_using, effective_gene = POS_EFF1, bE = 0.2, bint = 0.8, bG = 0.8, sigmaE = 1, sigma_eps = 0.5, MAF = MAF))
r1 <- error1 %>% filter(BP != POS_EFF1) %>% select(proposed_1:oracle_error)
r1 <- apply(r1<= 5 * (10 ^ -2), 2, mean)


#3:
suppressWarnings(error2 <- Compare_Aggreg_type1_error_auxiliary(G_using = G_using, POS_using = POS_using, effective_gene = POS_EFF2, bE = 0.2, bint = 0.8, bG = 0.8, sigmaE = 1, sigma_eps = 0.5, MAF = MAF))
r2 <- error2 %>% filter(BP != POS_EFF2) %>% select(proposed_1:oracle_error)
r2 <- apply(r2<=5 * (10 ^ -2), 2, mean)

#5:
suppressWarnings(error3 <- Compare_Aggreg_type1_error_auxiliary(G_using = G_using, POS_using = POS_using, effective_gene = POS_EFF3, bE = 0.2, bint = 0.8, bG = 0.8, sigmaE = 1, sigma_eps = 0.5, MAF = MAF))
r3 <- error3 %>% filter(BP != POS_EFF3) %>% select(proposed_1:oracle_error)
r3 <- apply(r3<=5 * (10 ^ -2), 2, mean)


r <- as_tibble(rbind(r1,r2,r3))
r$MAF <- sort(MAF[POS_using %in% c(POS_EFF1,POS_EFF2,POS_EFF3)])
r %>% pivot_longer(proposed_1:oracle_error, "Type", values_to = "Error") %>% ggplot(aes(MAF,Error,color = Type)) + geom_line() + ylab("Type I Error") + geom_hline(aes(yintercept = 0.05))
```
...
```{r}
all_error <- error3 %>% mutate(proposed_1 = proposed_1<=0.05, proposed_2 = proposed_2<=0.05, oracle_p = oracle_p<=0.05, oracle_error = oracle_error<=0.05)
all_error$MAF_level <- ifelse(all_error$MAF >= 0.1, "0.1-0.2","0.05-0.1")
all_error$MAF_level <- ifelse(all_error$MAF >= 0.2, "0.2-0.3", all_error$MAF_level)
all_error$MAF_level <- ifelse(all_error$MAF >= 0.3, "0.3-0.4", all_error$MAF_level)
all_error$MAF_level <- ifelse(all_error$MAF >= 0.4, "0.4-0.5", all_error$MAF_level)
...
Based on the above plot, all the procedures have type I error rate in the expected range, regardless of the MAF level.



```{r}
error3$MAF_level <- ifelse(error3$MAF >= 0.1, "0.1-0.2","0.05-0.1")
error3$MAF_level <- ifelse(error3$MAF >= 0.2, "0.2-0.3", error3$MAF_level)
error3$MAF_level <- ifelse(error3$MAF >= 0.3, "0.3-0.4", error3$MAF_level)
error3$MAF_level <- ifelse(error3$MAF >= 0.4, "0.4-0.5", error3$MAF_level)
error3 %>% pivot_longer(proposed_1:oracle_error, names_to = "Type", values_to = "p_value") %>% ggplot(aes(x = p_value, fill = Type)) + geom_histogram(bins = 5) + facet_grid(~MAF_level)
```
